...
namespace DependencyInjector
{
    ...
    */
    ...
    public class InjectorContainer : IRegister, IGet
    {
        ...
        {
            if (!typeof(T).IsClass)
                throw new InvalidTypeException($"Type {typeof(T).Name} must be class!");
        ...
        private static ParameterInfo[] GetParameters(ConstructorInfo[] constructors, List<int> argAmountList)
        {
            ...
            if (suitableCrots.Count() > 1)
                throw new ConstructorActivateException("There are more than 1 constructors with maximum number of parameters", new Exception());
        ...
        private static void BindTypeValidation<Interface, Class>()
        {
            if (!typeof(Interface).IsInterface)
                throw new InvalidTypeException($"Type {typeof(Interface).Name} must be interface!");

            if (!typeof(Class).IsClass)
                throw new InvalidTypeException($"Type {typeof(Class).Name} must be class!");

            if (!IsContain<Interface, Class>())
                throw new NoImplementedInterfaceException($"Interface {typeof(Interface).Name} not implemeted by class {typeof(Class).Name}");
        }
        ...
        private void IsTypeAlreadyBinded<T, U>()
        {
            if (IsExist(typeof(T), typeof(U)))
                throw new TypeRegistrationException($"Type {typeof(T).Name} is already binded!");
        }
        ...
        {
            if (dictionaryKey.ContainsKey(key))
                throw new KeyRegistrationException($"An element with the same key has already been added.");
        }
        ...
        private void NoRegisteredTypeValidation<T>()
        {
            if (!dictionary.ContainsKey(typeof(T)))
                throw new NoRegisteredKeyException($"No registered key {typeof(T).Name}");
        }
        ...
        {
            if (!dictionaryKey.ContainsKey(key))
                throw new NoRegisteredKeyException($"No registered key {key}");

            if (dictionaryKey[key].Key != typeof(Interface))
                throw new NoKeyTypeException($"No registered interface {typeof(Interface)}");
        }
        ...
        private static void KeyInitValidation(string key)
        {
            if (string.IsNullOrEmpty(key))
                throw new KeyNullOrEmptyException($"Key {key} must be initialised!");
        }
